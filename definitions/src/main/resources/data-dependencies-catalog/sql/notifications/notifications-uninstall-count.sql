...
FROM (SELECT shown_table.notification_type,
      ...
      FROM (SELECT s1.device.device_id AS device_id_purge
           ...
           (SELECT split(path,'/')[2] AS notification_type,
            ...
            AND   (jest(s2.event_data,'test_notification') = 'false' OR jest(s2.event_data,'test_notification') is NULL) -- Solo para que la comparativa con campaign_batch tenga sentido.
