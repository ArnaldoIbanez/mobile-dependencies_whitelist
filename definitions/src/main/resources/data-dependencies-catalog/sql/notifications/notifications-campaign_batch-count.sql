SELECT '2017-12-10' as fecha, COUNT(1) as total, temporal.campaign_batch, temporal.campaign_id, temporal.batch_id, 'android' as platform, temporal.site_id, temporal.marketplace
FROM (SELECT campaign_id,
      ...
      FROM (SELECT jest(s1.event_data,'device_id') AS device_id_purge
            FROM tracks AS s1
            WHERE s1.ds >= '2017-12-10'
            AND   s1.ds < '2017-12-12'
           ...
           (SELECT application.site_id AS site_id,
                   ...
                   ds
            FROM tracks s2
            WHERE s2.ds >= '2017-12-10'
            AND   s2.ds < '2017-12-11'
