SELECT summary.business AS marketplace, summary.event AS event, sum(summary.total) AS total, summary.fecha AS ds
FROM 
(SELECT application.business AS business, '@send_date' AS fecha, jest(event_data, 'event_type') AS event, 1 AS total
  ...
  UNION ALL
  SELECT application.business AS business, '@send_date' AS fecha, 'dead_devices' AS event, 1 AS total
  FROM tracks
  LEFT JOIN (
  ...
  GROUP BY application.business, '@send_date', 'dead_devices', device.device_id
) summary
GROUP BY summary.business, summary.event, summary.fecha