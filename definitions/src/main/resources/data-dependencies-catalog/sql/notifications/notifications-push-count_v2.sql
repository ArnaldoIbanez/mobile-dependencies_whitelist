SELECT summary.business AS marketplace, summary.event AS event, summary.total AS total, summary.fecha AS ds
FROM 
(SELECT application.business AS business, '@send_date' AS fecha, jest(event_data, 'event_type') AS event, sum(1) AS total
  FROM tracks
  WHERE ds >= '@send_date' 
