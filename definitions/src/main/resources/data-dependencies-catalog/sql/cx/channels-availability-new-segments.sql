SELECT Substr(tb.requested_datetime_minute, 1, 13) AS requested_datetime,
       ...
       tb.origin AS origin,
       tb.ml_seller_segment AS ml_seller_segment,
       tb.mp_seller_segment AS mp_seller_segment,
       tb.ml_buyer_segment AS ml_buyer_segment,
...
FROM
  (SELECT COALESCE(chattable.requested_datetime_minute, c2ctable.requested_datetime_minute, sactable.requested_datetime_minute) AS requested_datetime_minute,
          ...
          COALESCE(chattable.ml_seller_segment, c2ctable.ml_seller_segment, sactable.ml_seller_segment) AS ml_seller_segment,
          COALESCE(chattable.mp_seller_segment, c2ctable.mp_seller_segment, sactable.mp_seller_segment) AS mp_seller_segment,
          COALESCE(chattable.ml_buyer_segment, c2ctable.ml_buyer_segment, sactable.ml_buyer_segment) AS ml_buyer_segment,
   ...
   FROM
     (SELECT v1.id AS id,
             ...
             v3.ml_seller_segment AS ml_seller_segment,
             v3.mp_seller_segment AS mp_seller_segment,
             v3.ml_buyer_segment AS ml_buyer_segment,
      ...
      FROM cx_help_channels tj LATERAL VIEW json_tuple(tj.data, 'application', 'event_data', 'user_timestamp', 'user', 'id', 'path') v1 AS application,
                                            ...
                                            path LATERAL VIEW json_tuple(v1.application, 'app_id', 'business', 'site_id') v2 AS app_id,
                                                              business,
                                                              site_id LATERAL VIEW json_tuple(v1.event_data, 'problem_type', 'reason', 'origin', 'is_available', 'ml_seller_segment', 'mp_seller_segment', 'ml_buyer_segment', 'queue_id', 'environment', 'track_id') v3 AS problem_type,
                                                                                   reason,
                                                                                   origin,
                                                                                   is_available,
                                                                                   ml_seller_segment,
   ...
   GROUP BY COALESCE(chattable.requested_datetime_minute, c2ctable.requested_datetime_minute, sactable.requested_datetime_minute),
            ...
            COALESCE(chattable.ml_seller_segment, c2ctable.ml_seller_segment, sactable.ml_seller_segment),
            COALESCE(chattable.mp_seller_segment, c2ctable.mp_seller_segment, sactable.mp_seller_segment),
            COALESCE(chattable.ml_buyer_segment, c2ctable.ml_buyer_segment, sactable.ml_buyer_segment),
...
GROUP BY substr(tb.requested_datetime_minute, 1, 13),
         ...
         tb.problem_type,
         tb.origin,
         tb.ml_seller_segment,
         tb.mp_seller_segment,
         tb.ml_buyer_segment,
         tb.business,
         tb.chat_queue_id,
         tb.c2c_queue_id,
