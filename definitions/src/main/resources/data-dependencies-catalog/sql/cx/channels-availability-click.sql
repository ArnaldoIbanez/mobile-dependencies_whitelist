     ...
     (SELECT v1.id AS id,
      ...
      FROM cx_help_channels tj LATERAL VIEW json_tuple(tj.data, 'application', 'event_data', 'user_timestamp', 'user', 'id', 'path') v1 AS application,
                                            ...
                                            path LATERAL VIEW json_tuple(v1.application, 'app_id', 'business', 'site_id') v2 AS app_id,
                                                              business,
                                                              site_id LATERAL VIEW json_tuple(v1.event_data, 'problem_type', 'reason', 'origin', 'is_available', 'ml_seller_profile', 'mp_seller_profile', 'ml_buyer_profile', 'queue_id', 'environment', 'track_id', 'segment', 'source_id') v3 AS problem_type,
                                                                                   reason,
                                                                                   origin,
                                                                                   is_available,
                                                                                   ml_seller_profile,
