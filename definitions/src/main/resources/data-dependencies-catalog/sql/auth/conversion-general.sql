...
count(distinct (case when (c_hits_login > 0 and c_auth_success = 0 and c_auth_success_otp = 0 and c_auth_failure > 0 and c_auth_failure_captcha = 0 and c_auth_failure_captcha_first = 0 and c_recovery = 0 and c_registration = 0 and c_banner = 0) then d2id else null end)) as c_failure_100_pass,
count(distinct (case when (c_hits_login > 0 and c_auth_success = 0 and c_auth_success_otp = 0 and c_auth_failure = 0 and (c_auth_failure_captcha > 0 or c_auth_failure_captcha_first > 0) and c_recovery = 0 and c_registration = 0 and c_banner = 0) then d2id else null end)) as c_failure_100_captcha,
count(distinct (case when (c_hits_login > 0 and c_auth_success = 0 and c_auth_success_otp = 0 and c_auth_failure > 0 and (c_auth_failure_captcha > 0 or c_auth_failure_captcha_first > 0) and c_recovery = 0 and c_registration = 0 and c_banner = 0) then d2id else null end)) as c_failure_100_pass_captcha,
count(distinct (case when (c_hits_login > 0 and c_auth_success = 0 and c_auth_success_otp = 0 and (c_auth_failure > 0 or c_auth_failure_captcha > 0 or c_auth_failure_captcha_first > 0) and (c_recovery > 0 or c_registration > 0 or c_banner > 0)) then d2id else null end)) as c_failure_100_pass_captcha_drop
from (
	select
