...
tracks {
    ...
    "/floating_button/button_mounted" (platform: "/mobile/android", type: TrackType.View) {  // When the floating button service renders a visible view on the screen
        ...
        tooltip_buttons_count(required: true, type: PropertyType.Numeric, description: "Indicates the amount tooltip buttons that the user has enabled (in 'seller_collect' there is one for each collection method allowed in site)")
        side_buttons_count(required: true, type: PropertyType.Numeric, description: "Indicates the amount of side buttons that the user has enabled")
    }
    ...
    "/floating_button/button_tapped" (platform: "/mobile/android", type: TrackType.Event) { // A deeplinked button from the view has been triggered
        deeplink_action(required: true,type: PropertyType.String, values: ["point", "qr", "link", "configuration"], description: "If the button activation was triggered manually or by the scheduler")
    }
    ...
    "/gadgets/settings/floating_button/schedule_saved" (platform: "/mobile/android", type: TrackType.Event) { // The button service was started with a configurated schedule
        ...
        monday(required: true, type: PropertyType.Boolean, description: "Enabled on monday")
        tuesday(required: true, type: PropertyType.Boolean, description: "Enabled on tuesday")
        wednesday(required: true, type: PropertyType.Boolean, description: "Enabled on wednesday")
        thursday(required: true, type: PropertyType.Boolean, description: "Enabled on thursday")
        friday(required: true, type: PropertyType.Boolean, description: "Enabled on friday")
        saturday(required: true, type: PropertyType.Boolean, description: "Enabled on saturday")
        sunday(required: true, type: PropertyType.Boolean, description: "Enabled on sunday")
    }
