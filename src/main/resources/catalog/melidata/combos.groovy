...
tracks {
    ...
    def combos_info = objectSchemaDefinitions {
        ...
        item(required: false, type: PropertyType.String, description: "Unique string that identifies the recommendation from were the item comes from")
        items(required: true, description: "Item ids that are being added", type: PropertyType.ArrayList)
        action(required: false, type: PropertyType.String, description: "User action", values: ['add-to-cart', 'checked']),
        quantity(required: false, type: PropertyType.Number, description: "Items selected at the moment"),
        experiments(required: false, type: PropertyType.String, description: "Unique string that identifies the recommendation from were the item comes from")
    }
