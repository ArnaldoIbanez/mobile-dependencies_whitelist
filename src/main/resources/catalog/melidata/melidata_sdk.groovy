...
tracks {
    ...
    "/melidata/statistics"(platform: "/mobile", type: TrackType.Event) {
        ...
        stream_pending_tracks(type: PropertyType.Numeric, required: false)
        melidata_pending_tracks(type: PropertyType.Numeric, required: false)
        send_counter()
        database_size()
        tracks_counter()
        stream_tracks_counter(type: PropertyType.Numeric, required: false)
        melidata_tracks_counter(type: PropertyType.Numeric, required: false)
        average_ok_time()
        average_error_time()
