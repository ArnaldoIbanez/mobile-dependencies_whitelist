...
FROM (
 SELECT CASE WHEN others['fragment'] RLIKE 'EI:[0-9]+' THEN 'search'
         ...
         WHEN jest(others['fragment'], 'origin') RLIKE 'vip' THEN 'vip'
    END AS source,
    CASE WHEN others['fragment'] RLIKE 'EI:[0-9]+' THEN regexp_extract(others['fragment'], 'EI:([0-9]+)', 1)
         ...
         WHEN jest(others['fragment'], 'origin') RLIKE 'page_x' THEN 'link_page_x'
         WHEN jest(others['fragment'], 'origin') RLIKE 'vip' THEN 'link_vip'
    END AS placement,
