...
FROM (
  SELECT
    ds,
    jest(event_data,'review_id') as review_id,
    jest(event_data,'review_version') as review_version,
    jest(event_data,'review_status') as review_status,
    jest(event_data,'domain_id') as domain_id,
    jest(event_data,'publication_id') as publication_id,
    jest(event_data,'sheet_id') as sheet_id,
    jest(event_data,'time') as time,
    jest(event_data,'user_initials') as user_initials,
  ...
  LATERAL VIEW explode( split(jest(event_data,'sections_comments'),',')) tf2 as sections_comments
  WHERE ds > '@param01' AND ds < '@param02'  AND path = '/structure_data/product_reviewer/review'
) as A
INNER JOIN (
  SELECT
  jest(event_data,'review_id') as review_id,
   MAX(jest(event_data,'review_version')) as review_version
  FROM default.tracks
  WHERE ds > '@param01' AND ds < '@param02' AND path = '/structure_data/product_reviewer/review'
  GROUP BY jest(event_data,'review_id')
